...
export default function CrearPage() {
  ...
  const isFormValid = useMemo(
    () => !isNameInvalid && !isBirthInvalid && !isImageInvalid && !isDescInvalid,
    [isNameInvalid, isBirthInvalid, isImageInvalid, isDescInvalid]
  );
      ...
      <form onSubmit={onSubmit} className="space-y-3 max-w-xl">
        <div>
          ...
          <input
            ...
            className="border rounded w-full p-2 focus:outline-none focus:ring-2 focus:ring-offset-2"
          />
          {isNameInvalid && <p id="err-name" role="alert" className="text-sm text-red-600">El nombre es requerido</p>}
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="border rounded w-full p-2 focus:outline-none focus:ring-2 focus:ring-offset-2"
          />
          {isBirthInvalid && <p id="err-birth" role="alert" className="text-sm text-red-600">La fecha es requerida</p>}
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="border rounded w-full p-2 focus:outline-none focus:ring-2 focus:ring-offset-2"
          />
          {isImageInvalid && <p id="err-image" role="alert" className="text-sm text-red-600">La imagen es requerida</p>}
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            className="border rounded w-full p-2 focus:outline-none focus:ring-2 focus:ring-offset-2"
          />
          {isDescInvalid && <p id="err-desc" role="alert" className="text-sm text-red-600">La descripción es requerida</p>}
        </div>

        <button
          disabled={!isFormValid}
          className="border rounded px-4 py-2 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-offset-2"
        >
          Crear
        </button>
      </form>
